...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
            // TODO(DONE): Implement this constructor in Step 5.
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        PriorityQueue<Path> frontierPQ = new PriorityQueue<Path>();
        Hashtable<T, Vertex> visitedTable = new Hashtable<T, Vertex>();
        Vertex startVertex = this.vertices.get(start);
        Vertex endVertex = this.vertices.get(end);

        // Check to see if the given start and end vertices are present in the graph
        if (startVertex != null && endVertex != null) {
            visitedTable.put(start, startVertex); // Add start vertext to the visited table

            for (Edge edge : startVertex.edgesLeaving) { // Add edges leaving from the start vertex to frontierPQ
                Path newPath = new Path(startVertex);
                newPath = new Path(newPath, edge);
                frontierPQ.add(newPath);
            }

            // Run while loop until there are no more vertices left to visit or no more edges to compare
            while ((this.vertices.size() != visitedTable.size()) && (frontierPQ.size() != 0)) {
                Path lowestCostPath = frontierPQ.poll(); // Remove the lowest cost path from the frontierPQ
                Vertex lowestCostTarget = lowestCostPath.end;
                if(!(visitedTable.containsKey(lowestCostTarget.data))) {
                    // Adding the end vertex of lowest cost path to the visited table
                    visitedTable.put(lowestCostTarget.data, lowestCostTarget);
                    if (lowestCostPath.end.equals(endVertex)) {
                        // If the end vertex of the lowest cost path is the given end vertex, then return this path
                        return lowestCostPath;
                    } else {
                        // Add all the edges leaving from this vertex to frontierPQ
                        for (Edge edge : lowestCostTarget.edgesLeaving) {
                            Path extPath = new Path(lowestCostPath, edge);
                            frontierPQ.add(extPath);
                        }
                    }
                }
            }
        ...
        throw new NoSuchElementException("No path was found.");// TODO(DONE): Implement this method in Step 7.
    }
